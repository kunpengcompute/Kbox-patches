...
# 调用方在 source base_box.sh 后需主动调用此函数
...
# 挂载后钩子：在 bb_start_box 完成 img 挂载、storage_size 写入后调用，
# 在容器启动之前。默认 no-op，调用方可覆盖此函数执行自定义数据初始化。
function bb_post_mount_hook() {
    :
}
...
function bb_check_wait_cmd_result() {
    ...
    local result=$2
    if [ "$result" == "-1" ]; then
        echo "cmd \"${cmd}\" bb_wait_cmd timeout"
    fi
}
...
function bb_check_system_size_modify() {
    local target_path="$1"
...
function bb_create_default_prop() {
    if [ -f $DEFAULT_PROP ]; then
        ...
        if bb_has_amd_gpu && [ ${ENABLE_SOFT_RENDER} -ne 1 ]; then
            ...
            bb_log_info "${DOCKER_NAME_PREFIX}$container_id vsyncoffset=${vsync_offset}"
...
function bb_create_box() {
    ...
    RUN_OPTION+=" --volume=$(bb_get_lxcfs_path)/proc/diskstats:/proc/diskstats:ro "
    RUN_OPTION+=" --volume=$(bb_get_lxcfs_path)/proc/meminfo:/proc/meminfo:ro "
    RUN_OPTION+=" --volume=$(bb_get_lxcfs_path)/proc/stat:/proc/stat:ro "
    ...
    if [ -f "$LOCAL_PROP" ]; then
        chmod 600 $LOCAL_PROP
        RUN_OPTION+=" --volume=$LOCAL_PROP:/data/local.prop:rw "
    fi
    ...
    if [ ${BB_HW_AMD_IS_W6800} -eq 1 ]; then
...
function bb_restart_box() {
    ...
    do
        ...
        local cmd="$RUNTIME_CMD exec -i ${BOX_NAME} logcat -d |grep \"addInterfaceToNetwork() failed\" > /dev/null 2>&1 &"
        local result=$(bb_wait_async_cmd "${cmd}")
        if [ "${result}" == "-1" ];then
            echo "${BOX_NAME} bb_wait_async_cmd logcat timeout"
        elif [ "${result}" != "0" ];then
            # 无异常日志(grep未匹配,退出码非0), 且检查关键进程均无异常，退出重启流程
