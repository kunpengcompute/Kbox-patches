...
function apply_only_64_compile_patchs()
{
    ...
    source $CURRENT_DIR/patch_config.sh
    local enable_only64_compile=1
    grep -q "ENABLE_ONLY_64BIT_COMPILE := true" $AOSP_PATH/vendor/kbox/products/kbox_arm64.mk || enable_only64_compile=0
    if [ $enable_only64_compile -eq 1 ]; then
        cd "$PATCH_DIR" || error "无法切换到 $PATCH_DIR 目录"
        for patch_name in "${android_only64_patch[@]}"; do
            echo "正在合入补丁 $patch_name ..."
            if ! apply_patch "$patch_name"; then
                error "错误：无法合入补丁 $patch_name."
            fi
