...
| 预置条件
 | Kbox云手机基本环境已部署完成。
                                                                                                                                                  |
| 测试步骤
 | 执行创建Kbox云手机容器命令./android11_kbox.sh start kbox_image:tag x y。 说明： 其中x，y表示想要创建的容器编号首尾值，如创建1到9号容器则x输入1，y输入9；如果只想创建单个容器，仅输入x即可。

执行命令docker ps -a查看已创建成功的Kbox云手机容器及状态。
 |
| 预期结果
 | Kbox云手机容器创建结束回显成功标志。执行命令可以查询到已创建的设备及状态。
                                                                                                                            |
| 测试结果
 |                                                                                                                                                                     |
| 备注
