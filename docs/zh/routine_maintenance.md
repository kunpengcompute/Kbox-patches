...
>
>CPU使用率和内存使用率超过总量的80%时，可能会出现容器响应卡顿的情况，此时建议清理后台应用。

## 看管/记录<a id="ZH-CN_TOPIC_0000002549825875"></a>
...
![htop看管/记录界面](./figures/zh-cn_image_0000002518346036.png)
...
|4|在iBMC升级过程中异常掉电| 升级iBMC过程中AC掉电可能导致iBMC硬件损坏，d导致服务器管理页面无法登录，需要重新更换iBMC硬件。 |Critical| 必须在客户同意的维测时段操作。必须由维护人员执行。必须得到客户的同意后才可操作。 |必须由维护人员执行或得到其同意。|
...
|1|Kbox云手机容器性能|APP启动时间| 启动并退出酷狗音乐1000次，记录每一次的启动时间，启动时间保持稳定，符合预期数值。 |
