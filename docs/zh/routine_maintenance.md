...
## 运维概述<a name="ZH-CN_TOPIC_0000002549705313"></a>
...
- 运维功能
    ...
    提供对运维对象的巡检、看管/记录、日志管理、高危操作和日常维护等能力。

## 巡检<a name="ZH-CN_TOPIC_0000002518185490"></a>

### 简介<a name="ZH-CN_TOPIC_0000002518185546"></a>

本章主要介绍对Kbox云手机容器及其运行环境的巡检操作，检查Kbox云手机容器部署环境，实时看管/记录容器运行状况及其资源占用，及时发现并处理影响容器正常运行的问题，保证Kbox云手机容器稳定运行。

### 巡检项目和周期<a name="ZH-CN_TOPIC_0000002518345464"></a>
...
### 检查容器状态<a name="ZH-CN_TOPIC_0000002549825335"></a>

#### 启动时容器状态<a name="ZH-CN_TOPIC_0000002518185466"></a>
...
#### 运行时容器状态<a name="ZH-CN_TOPIC_0000002549825301"></a>
...
### 检查容器资源消耗<a name="ZH-CN_TOPIC_0000002549705259"></a>
...
## 看管/记录<a name="ZH-CN_TOPIC_0000002549705277"></a>

### 简介<a name="ZH-CN_TOPIC_0000002518345484"></a>

本章主要介绍Kbox云手机容器在运行过程中对服务器系统资源消耗的看管/记录方法，让用户及时了解系统资源使用情况、趋势和告警。

### CPU使用详情<a name="ZH-CN_TOPIC_0000002518345436"></a>
...
通过该命令，可以看管/记录Kbox云手机容器中是否有CPU占用率过高的进程，若有，则需要进一步排查该进程是否异常。
...
### 系统内存<a name="ZH-CN_TOPIC_0000002518185482"></a>
...
### 系统存储<a name="ZH-CN_TOPIC_0000002549705335"></a>
...
如下图所示Kbox云手机容器数据存储在“/root/mount/data/”下，红色矩形中为容器kbox_1的数据存储信息，其存储大小为“16GB”，当前使用率为“1%”。若“Use%”的值不大于85%，则磁盘空间使用率正常，否则需要清理磁盘空间。

![](./figures/zh-cn_image_0000002518185698.png)

### GPU使用详情<a name="ZH-CN_TOPIC_0000002549705267"></a>

#### AMD GPU状态查询<a name="ZH-CN_TOPIC_0000002518185500"></a>
...
在Kbox云手机容器运行过程中，若GPU温度过高，可能会导致服务器宕机的异常情况，因此在运行大型游戏或应用时，需要看管/记录运行环境上GPU的温度。
...
#### 道客DC1000/DC1000C状态查询<a name="ZH-CN_TOPIC_0000002518185532"></a>

使用GPU驱动包VAGPU-A15-C-F-26.02.06.00.RC2.tgz中提供的工具，以查看GPU状态。

1. 请参见《install guide》文档中“环境准备”章节中的“[软件环境](https://gitcode.com/boostkit/Kbox-patches/blob/AOSP15/docs/zh/install_guide.md#软件环境)”小节获取VAGPU-A15-C-F-26.02.06.00.RC2.tgz包并解压，将解压获得的显卡工具包VAGPU-A15-C-F-26.02.06.00.RC2/tools下的工具上传至服务器。
2. 显卡工具包tools的具体使用方法可以请参见VAGPU-A15-C-F-26.02.06.00.RC2/docs/tools_guide中的说明文档。

## 日志管理<a name="ZH-CN_TOPIC_0000002518345474"></a>

### 简介<a name="ZH-CN_TOPIC_0000002518185510"></a>

本章主要介绍在Kbox云手机容器运行过程中的日志管理方法，包括日志查询、转储等。

### Kbox云手机容器日志<a name="ZH-CN_TOPIC_0000002518345396"></a>

#### 容器元数据<a name="ZH-CN_TOPIC_0000002518345386"></a>
...
#### 容器内logcat日志<a name="ZH-CN_TOPIC_0000002549825243"></a>
...
#### 容器内进程信息<a name="ZH-CN_TOPIC_0000002518345428"></a>
...
#### 容器内top信息<a name="ZH-CN_TOPIC_0000002549825273"></a>
...
#### ANR时应用堆栈信息<a name="ZH-CN_TOPIC_0000002549825251"></a>

在Kbox云手机容器内若出现了应用程序未响应（ANR，Application Not Responding）时，需要搜集相关应用堆栈信息，该信息保存在容器的“/data/anr/”路径下。

#### dumpsys信息<a name="ZH-CN_TOPIC_0000002549825235"></a>
...
#### 容器属性信息<a name="ZH-CN_TOPIC_0000002518345450"></a>
...
### 运行环境日志<a name="ZH-CN_TOPIC_0000002549705299"></a>

#### dmesg日志<a name="ZH-CN_TOPIC_0000002549825261"></a>
...
### 维护工具<a name="ZH-CN_TOPIC_0000002518185564"></a>

#### 工具简介<a name="ZH-CN_TOPIC_0000002549825293"></a>

为增强云手机原型的可测试性、可服务性和可维护性，Kbox云手机容器提供了Kbox_maintainer（维护工具）。该工具集成了日志收集、资源检查、故障恢复等多项功能，显著提升了云手机原型的整体性能。

>![](public_sys-resources/icon-note.gif) **说明：** 
>
>Kbox_maintainer工具包含在Kbox_AOSP15.zip中，Kbox_AOSP15.zip的获取方式请参见《install guide》文档中“环境准备”章节中的“[软件环境](https://gitcode.com/boostkit/Kbox-patches/blob/AOSP15/docs/zh/install_guide.md#软件环境)”小节。

#### 日志收集<a name="ZH-CN_TOPIC_0000002518345380"></a>

Kbox_maintainer（维护工具）收集的日志信息包括Android日志和服务器日志，请参考[**表 1** Kbox_maintainer日志收集](#Kbox_maintainer日志收集)。
...
Kbox_maintainer（维护工具）支持单个容器和全部容器日志收集，收集后打包。提供**log**命令字段，可通过容器号或容器名参数，收集指定容器的日志。不带容器号，默认收集所有容器日志。例如：
...
#### 资源检查<a name="ZH-CN_TOPIC_0000002518345418"></a>

Kbox_maintainer（维护工具）提供资源检查功能，收集Kbox云手机容器及服务器的内存、CPU、存储和GPU等信息，单独查询某项信息时请参见[**表 1** Kbox_maintainer资源检查](#Kbox_maintainer资源检查)详情中的命令（容器内运行相关命令即可），或者使用Kbox_maintainer工具批量收集所有数据。
...
Kbox_maintainer（维护工具）支持单个容器和全部容器资源信息收集，收集后打包。提供**resource**命令字段，可通过容器号或容器名参数，收集指定容器的资源情况。不带容器号，默认收集所有容器资源情况。例如：
...
#### 故障检查及恢复<a name="ZH-CN_TOPIC_0000002518185554"></a>

Kbox_maintainer（维护工具）支持查看Kbox云手机容器的服务状态，用于云手机日常的巡检和恢复，请参考[**表 1** Kbox_maintainer故障检查及恢复](#Kbox_maintainer故障检查及恢复)。
...
Kbox_maintainer（维护工具）提供**check**命令字段，可通过容器号或容器名参数，检查指定容器的服务状态。不带容器号，默认检查所有容器状态。例如：
...
Kbox_maintainer（维护工具）提供**recover**命令字段，可通过容器号或容器名参数，恢复指定容器的服务状态。不带容器号，默认恢复所有容器状态。例如：
...
## 高危操作<a name="ZH-CN_TOPIC_0000002549705289"></a>
...
## 日常运维<a name="ZH-CN_TOPIC_0000002518345408"></a>

### Docker信息查询<a name="ZH-CN_TOPIC_0000002549705323"></a>
...
### Android信息查询<a name="ZH-CN_TOPIC_0000002518345374"></a>
...
### 服务器信息查询<a name="ZH-CN_TOPIC_0000002549705233"></a>
...
> ![](public_sys-resources/icon-note.gif) **说明：** 
>
> 当Host OS为openEuler 24.03 LTS SP1时，联网游戏场景需要打开防火墙的某些端口（端口号由游戏厂商定义，如王者荣耀使用的端口为50012），否则会出现网络异常无法登录游戏的情况。防火墙配置方法如下。
>
> 1. 启用50012端口，用于王者荣耀。
>
>    ```bash
...
## 参考信息<a name="ZH-CN_TOPIC_0000002549825283"></a>

### 性能指标参考<a name="ZH-CN_TOPIC_0000002549825325"></a>
