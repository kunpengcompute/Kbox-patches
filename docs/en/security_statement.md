...
1. Define separated permissions to allocate only necessary permissions to each user and develop a comprehensive user management mechanism. This prevents administrator or operator accounts of the cloud phone management system from being forged. In addition, key system operations must be logged for audits. Do not assign the system administrator, security administrator, and security auditor roles to the same person.
